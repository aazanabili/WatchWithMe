...
function Usage { @('Usage: watch-with-me <command>','  start [--no-build] | stop | restart [--no-build] | status | health','  logs [web|realtime|postgres|redis|minio|livekit|coturn|media-worker] [--follow] [--tail N]','  remove | clean [--yes] | test | doctor | config | help') | Write-Output }
