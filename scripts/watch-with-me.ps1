...
function Usage { @('Usage: watch-with-me <command>','  start [--no-build] | stop | restart [--no-build] | status | health','  logs [web|realtime|postgres|redis] [--follow] [--tail N]','  remove | clean [--yes] | test | doctor | config | help') | Write-Output }
...
switch ($Command) {
  ...
  'start' { Preflight; $build=$true; if ($Args -contains '--no-build') {$build=$false;$Args=@($Args|Where-Object{$_ -ne '--no-build'})}; if($Args.Count){Fail 'start accepts only --no-build.'}; if($build){Compose @('up','--build','-d')}else{Compose @('up','-d')}; WaitReady; break }
  'stop' { Preflight; if($Args.Count){Fail 'stop takes no arguments.'}; Compose @('stop'); break }
  'restart' { Preflight; $build=$true; if ($Args -contains '--no-build') {$build=$false;$Args=@($Args|Where-Object{$_ -ne '--no-build'})}; if($Args.Count){Fail 'restart accepts only --no-build.'}; Compose @('stop'); if($build){Compose @('up','--build','-d')}else{Compose @('up','-d')}; WaitReady; break }
  ...
  'logs' { Preflight; if($Args.Count -lt 1 -or $Args[0] -notin $Services){Fail 'logs requires one allowed service: web, realtime, postgres, redis.'}; $service=$Args[0]; $follow=$false;$tail='100'; for($i=1;$i -lt $Args.Count;$i++){switch($Args[$i]){'--follow'{$follow=$true};'--tail'{if(++$i -ge $Args.Count -or $Args[$i] -notmatch '^[0-9]+$'){Fail '--tail requires a non-negative integer.'};$tail=$Args[$i]};default{Fail 'unknown logs option.'}}}; $a=@('logs','--tail',$tail);if($follow){$a+='--follow'};$a+=$service;Compose $a;break }
